...
#define int                         ll
#define dd                          double
#define arhan_c();              ios_base::sync_with_stdio(0); cin.tie(0); cout.tie(0)
...
signed main()
{
	...
	vr seq(10000001);
		...
		f2(1,i)
		{
		    ...
		    pre[j-1]=pre[j-2]+seq[j];
		}
		f2(1,i)
		{
		    mul[j]=(modadd(mul[j-1],modmul(j*j,seq[j],M1),M1));
		}
	...
	while(t--)
	{
		...
		ll x=lb(pre,l-1)-pre.begin();
		ll y=lb(pre,r)-pre.begin();
		//debug2(x,y);
		ll ans1=(modadd(mul[x],modmul(l-1-((x==0)? 0 : pre[x-1]),modmul(x+1,x+1,M1),M1),M1));
		ll ans2=(modadd(mul[y],modmul(r-((y==0)? 0:pre[y-1]),modmul(y+1,y+1,M1),M1),M1));
		//debug2(ans1,ans2);
		co(modsub(ans2,ans1,M1));
