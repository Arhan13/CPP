...
ll BinaryConcatenation (ll x, ll y) {
    ll pos1=0,pos2=0;
    string X = bitset<40>(x).to_string();
    string Y = bitset<40>(y).to_string();
    ...
    string FX = X.substr(pos1, X.length());
    string FY = Y.substr(pos2, Y.length());

    string xpy = FX+FY;
    string ypx = FY+FX;

    ll xpyv = stoll(xpy, nullptr, 2);
    ll ypxv = stoll(ypx, nullptr, 2);
...
int main() {
    ...
    while(t--) {
        ...
        for(int i=0;i<n;i++) {
            for(int j=0;j<n;j++) {
                if(i!=j) {
                    ll value = BinaryConcatenation(arr[i], arr[j]);
                    if(value>maximum) {
                        maximum = value;
                    }
                }
